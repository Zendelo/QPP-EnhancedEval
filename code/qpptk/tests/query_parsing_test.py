...
def test_end_to_end_with_porter_stemmer():
...
def test_end_to_end_without_stemmer():
    with tempfile.TemporaryDirectory() as out_dir, tempfile.TemporaryDirectory() as stats_dir:
        expected = [
        ...
        args = parse_args(['-ti', 'tests/resources/small-example-02/index', '--jsonl_queries', 'tests/resources/small-example-01/queries.jsonl', '--predict', '--retrieve', '--output', out_dir, '--cleanOutput','--stats_index_path',  stats_dir])
        actual = get_queries_object(args)
        assert expected[0] == actual.get_query(qid='1')
        assert expected[1] == actual.get_query(qid='2')
        assert expected[2] == actual.get_query(qid='3')